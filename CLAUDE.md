...
- **Python is the public surface.** Target **3.10+** — the Rust bindings use `abi3-py310` and CI runs 3.10/3.11/3.12. The only runtime dependency is `numpy`; heavier libs (`scipy`, `shapely`, `geopandas`, …) appear only in the optional `examples`/notebook extras in `pyproject.toml`. Lint with **flake8**: CI fails on `flake8 mortie --select=E9,F63,F7,F82`, and runs a non-blocking style pass at `--max-line-length=88`; ruff is configured.
- Docstrings are numpydoc, and the standard is enforced mechanically (issue #140): `numpydoc lint mortie/*.py` runs as a **hard gate** in `lint.yml`, with the curated check set in `[tool.numpydoc_validation]` (PR01/PR02/PR03/PR04/PR09, RT01, SS01 — signature agreement and section presence; the filler-forcing completeness rules are deliberately off, and private members/dunders/tests are excluded). Every public function, class and method carries a numpydoc docstring: one-line summary, then Parameters / Returns / Raises where applicable. Narrative rationale belongs in the Extended Summary, not forced into a parameter table. This is what mkdocstrings renders on the docs site (docstring_style: numpy) — a freeform docstring publishes as an undifferentiated block.
...
- **The Rust extension is the sole runtime path — there are no pure-Python fallbacks.** The `MORTIE_FORCE_PYTHON`-gated Python twins of `fastNorm2Mort`, `VaexNorm2Mort`, `geo2mort`, `mort2norm` (`tools.py`) and `split_children` (`prefix_trie.py`) have been removed; those functions call Rust unconditionally and a missing extension fails loudly at import. **All functionality (coverage, MOC, buffer, linestring, the inverse/encode API) is Rust-only** — test it against expected output directly (golden/pinned fixtures captured from the Rust path), and don't add a Python fallback unless asked.
